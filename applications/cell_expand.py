...
#class P1_Structure(Structure):
#    def  __init__(self, cell_structure=None):
#       if isinstance(cell_structure, Structure):
#         self.structure=cell_structure 
#       else: 
#         raise RuntimeError, "expected instance of Structure"
#       return
       
# the function expand the cell using the space group given and expand to a supercell using given l,m,n, it doesn't change the unitcell for the lattice. Constraints are return in one string.     
def  structure_all_expand(spcgr, init_structure=None, l=1, m=1, n=1):
...
#        if not atom.element in scatterertype_list:
#            scatterertype_list.append(atom.element)
...
# step 2: write the scatterers (atoms)
    ...
    a=Structure(allatoms, ini_lat)
    a.allconstrain=allconstrain
    return a 

# This function only expand the structure using the space group,constraints are returned.    
...
# step 2: write the scatterers (atoms)
    ...
    a=Structure(unitcell_atoms, ini_lat)
    a.allconstrain=list_conts
    return a     
...
def  super_structure_expand(unitcell_structure=None, l=1, m=1, n=1):
    ...
    supercell=Structure(allatoms, super_lattice)
    supercell.allconstrain=allconstrain  
    return supercell      
